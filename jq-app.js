! function(window, document, undefined) {
  ...
  function copy(source, destination) {
    function copyRecurse(source, destination) {
      ...
      if (isArray(source))
        for (var i = 0, ii = source.length; ii > i; i++)
        ...
        for (key in source)
          hasOwnProperty.call(source, key) && (destination[key] = copyElement(source[key]));
    ...
    function copyElement(source) {
      ...
      return isArray(source) ? (destination = [],
          needsRecurse = !0) : isTypedArray(source) ? destination = new source.constructor(source) : isDate(source) ? destination = new Date(source.getTime()) : isRegExp(source) ? (destination = new RegExp(source.source, source.toString()
            .match(/[^\/]*$/)[0]),
        ...
        needsRecurse ? copyRecurse(source, destination) : destination
    }
    var stackSource = [],
      stackDest = [];
    if (destination) {
  ...
  function equals(o1, o2) {
    ...
    if (t1 == t2 && "object" == t1) {
      if (!isArray(o1)) {
        ...
        if (isRegExp(o1))
          return isRegExp(o2) ? o1.toString() == o2.toString() : !1;
  ...
  function parseKeyValue(keyValue) {
    var obj = {};
    return forEach((keyValue || "")
        .split("&"),
        function(keyValue) {
          var splitPoint, key, val;
          keyValue && (key = keyValue = keyValue.replace(/\+/g, "%20"),
            splitPoint = keyValue.indexOf("="), -1 !== splitPoint && (key = keyValue.substring(0, splitPoint),
              val = keyValue.substring(splitPoint + 1)),
            key = tryDecodeURIComponent(key),
            isDefined(key) && (val = isDefined(val) ? tryDecodeURIComponent(val) : !0,
          ...
          jQuery.cleanData = function(elems) {
            ...
            if (skipDestroyOnNextJQueryCleanData)
              ...
              for (var elem, i = 0; null != (elem = elems[i]); i++)
                ...
                events && events.$destroy && jQuery(elem)
  ...
  function toDebugString(obj) {
    return "function" == typeof obj ? obj.toString()
      .replace(/ \{[\s\S]*$/, "") : isUndefined(obj) ? "undefined" : "string" != typeof obj ? serializeObject(obj) : obj
  }
  ...
  function JQLite(element) {
    ...
    if (isString(element) && (element = trim(element),
        argIsString = !0), !(this instanceof JQLite)) {
      ...
      if (type) {
        var removeHandler = function(type) {
          ...
          isDefined(fn) && arrayRemove(listenerFns || [], fn),
            isDefined(fn) && listenerFns && listenerFns.length > 0 || (removeEventListenerFn(element, type, handle),
              delete events[type])
        };
  ...
  function jqLiteDocumentLoaded(action, win) {
    win = win || window,
      "complete" === win.document.readyState ? win.setTimeout(action) : jqLite(win)
      .on("load", action)
  }
      ...
      this.$get = ["$window", "$location", "$rootScope", function($window, $location, $rootScope) {
        ...
        function getYOffset() {
          ...
          if (isFunction(offset))
            ...
            offset = "fixed" !== style.position ? 0 : elem.getBoundingClientRect()
              .bottom
          } else
  ...
  function Browser(window, document, $log, $sniffer) {
      ...
      self.url = function(url, replace, state) {
        if (isUndefined(state) && (state = null),
          ...
          return lastBrowserUrl = url,
            lastHistoryState = state, !$sniffer.history || sameBase && sameState ? ((!sameBase || pendingLocation) && (pendingLocation = url),
    ...
    self.onUrlChange = function(callback) {
        return urlChangeInit || ($sniffer.history && jqLite(window)
            .on("popstate", cacheStateAndFireUrlChange),
            jqLite(window)
      ...
      self.$$applicationDestroyed = function() {
        jqLite(window)
          .off("hashchange popstate", cacheStateAndFireUrlChange)
      },
    ...
    this.debugInfoEnabled = function(enabled) {
      ...
      this.$get = ["$injector", "$interpolate", "$exceptionHandler", "$templateRequest", "$parse", "$controller", "$rootScope", "$document", "$sce", "$animate", "$$sanitizeUri", function($injector, $interpolate, $exceptionHandler, $templateRequest, $parse, $controller, $rootScope, $document, $sce, $animate, $$sanitizeUri) {
        ...
        function compile($compileNodes, transcludeFn, maxPriority, ignoreDirective, previousCompileContext) {
          $compileNodes instanceof jqLite || ($compileNodes = jqLite($compileNodes)),
            forEach($compileNodes, function(node, index) {
              node.nodeType == NODE_TYPE_TEXT && node.nodeValue.match(/\S+/) && ($compileNodes[index] = jqLite(node)
                .wrap("<span></span>")
                .parent()[0])
            });
          ...
          return function(scope, cloneConnectFn, options) {
            ...
            var $linkNode;
            if ($linkNode = "html" !== namespace ? jqLite(wrapTemplate(namespace, jqLite("<div>")
                .append($compileNodes)
                .html())) : cloneConnectFn ? JQLitePrototype.clone.call($compileNodes) : $compileNodes,
        ...
        function detectNamespaceForChildElements(parentElement) {
          ...
          return node && "foreignobject" !== nodeName_(node) && node.toString()
            .match(/SVG/) ? "svg" : "html"
        }
        ...
        function compileNodes(nodeList, transcludeFn, $rootElement, maxPriority, ignoreDirective, previousCompileContext) {
          ...
          for (var attrs, directives, nodeLinkFn, childNodes, childLinkFn, linkFnFound, nodeLinkFnFound, linkFns = [], i = 0; i < nodeList.length; i++)
            ...
            childLinkFn = nodeLinkFn && nodeLinkFn.terminal || !(childNodes = nodeList[i].childNodes) || !childNodes.length ? null : compileNodes(childNodes, nodeLinkFn ? (nodeLinkFn.transcludeOnThisElement || !nodeLinkFn.templateOnThisElement) && nodeLinkFn.transclude : transcludeFn), (nodeLinkFn || childLinkFn) && (linkFns.push(i, nodeLinkFn, childLinkFn),
              linkFnFound = !0,
        ...
        function collectDirectives(node, directives, attrs, maxPriority, ignoreDirective) {
          ...
          switch (nodeType) {
            ...
            case NODE_TYPE_TEXT:
              if (11 === msie)
                for (; node.parentNode && node.nextSibling && node.nextSibling.nodeType === NODE_TYPE_TEXT;)
        ...
        function applyDirectivesToNode(directives, compileNode, templateAttrs, transcludeFn, jqCollection, originalReplaceDirective, preLinkFns, postLinkFns, previousCompileContext) {
          ...
          function getControllers(directiveName, require, $element, elementControllers) {
            ...
            if (isString(require)) {
              ...
              value = [];
              for (var i = 0, ii = require.length; ii > i; i++)
          ...
          for (var directive, directiveName, $template, linkFn, directiveValue, terminalPriority = -Number.MAX_VALUE, newScopeDirective = previousCompileContext.newScopeDirective, controllerDirectives = previousCompileContext.controllerDirectives, newIsolateScopeDirective = previousCompileContext.newIsolateScopeDirective, templateDirective = previousCompileContext.templateDirective, nonTlbTranscludeDirective = previousCompileContext.nonTlbTranscludeDirective, hasTranscludeDirective = !1, hasTemplate = !1, hasElementTranscludeDirective = previousCompileContext.hasElementTranscludeDirective, $compileNode = templateAttrs.$$element = jqLite(compileNode), replaceDirective = originalReplaceDirective, childTranscludeFn = transcludeFn, i = 0, ii = directives.length; ii > i; i++) {
            ...
            if ((directiveValue = directive.scope) && (directive.templateUrl || (isObject(directiveValue) ? (assertNoDuplicate("new/isolated scope", newIsolateScopeDirective || newScopeDirective, directive, $compileNode),
              ...
              directiveName = directive.name, !directive.templateUrl && directive.controller && (directiveValue = directive.controller,
                ...
                return {
                  pre: function(scope, element, attr) {
                    ...
                    newValue !== value && (interpolateFn = newValue && $interpolate(newValue, !0, trustedContext, allOrNothing),
                        value = newValue),
                      interpolateFn && (attr[name] = interpolateFn(scope), ($$observers[name] || ($$observers[name] = []))
                        .$$inter = !0, (attr.$$observers && attr.$$observers[name].$$scope || scope)
        ...
        function replaceWith($rootElement, elementsToRemove, newNode) {
          ...
          fragment.appendChild(firstElementToRemove),
            jqLite.hasData(firstElementToRemove) && (jqLite.data(newNode, jqLite.data(firstElementToRemove)),
        ...
        function initializeDirectiveBindings(scope, attrs, destination, bindings, directive) {
          ...
          return forEach(bindings, function(definition, scopeName) {
                  ...
                  parentGet = $parse(attrs[attrName]),
                    ...
                    parentSet = parentGet.assign || function() {
                      throw lastValue = destination[scopeName] = parentGet(scope),
                        $compileMinErr("nonassign", "Expression '{0}' used with directive '{1}' is non-assignable!", attrs[attrName], directive.name)
                    },
            ...
            removeWatchCollection.length && function() {
              for (var i = 0, ii = removeWatchCollection.length; ii > i; ++i)
        ...
        Attributes.prototype = {
          ...
          $set: function(key, value, writeAttr, attrName) {
            ...
            if (booleanKey ? (this.$$element.prop(key, value),
              ...
              var lastTuple = trim(rawUris[2 * i])
                .split(/\s/);
      ...
      this.$get = ["$injector", "$window", function($injector, $window) {
        ...
        return function(expression, locals, later, ident) {
          ...
          if (later = later === !0,
            ...
            isString(expression)) {
            if (match = expression.match(CNTRL_REG), !match)
          ...
          if (later) {
            var controllerPrototype = (isArray(expression) ? expression[expression.length - 1] : expression)
              .prototype;
  ...
  function $HttpProvider() {
    ...
    this.useLegacyPromiseExtensions = function(value) {
      return isDefined(value) ? (useLegacyPromise = !!value,
    ...
    this.$get = ["$httpBackend", "$$cookieReader", "$cacheFactory", "$rootScope", "$q", "$injector", function($httpBackend, $$cookieReader, $cacheFactory, $rootScope, $q, $injector) {
      function $http(requestConfig) {
        ...
        function executeHeaderFns(headers, config) {
          ...
          return forEach(headers, function(headerFn, header) {
              isFunction(headerFn) ? (headerContent = headerFn(config),
      ...
      function sendReq(config, reqData) {
        ...
        if ($http.pendingRequests.push(config),
          promise.then(removePendingReq, removePendingReq), !config.cache && !defaults.cache || config.cache === !1 || "GET" !== config.method && "JSONP" !== config.method || (cache = isObject(config.cache) ? config.cache : isObject(defaults.cache) ? defaults.cache : defaultCache),
      ...
      if ($browser.$$incOutstandingRequestCount(),
        ...
        xhr.open(method, url, !0),
          ...
          xhr.onload = function() {
            ...
            0 === status && (status = response ? 200 : "file" == urlResolve(url)
                .protocol ? 404 : 0),
  ...
  function LocationHtml5Url(appBase, appBaseNoFile, basePrefix) {
    this.$$html5 = !0,
      ...
      this.$$parseLinkUrl = function(url, relHref) {
        ...
        return isDefined(appUrl = beginsWith(appBase, url)) ? (prevAppUrl = appUrl,
            rewrittenUrl = isDefined(appUrl = beginsWith(basePrefix, appUrl)) ? appBaseNoFile + (beginsWith("/", appUrl) || appUrl) : appBase + prevAppUrl) : isDefined(appUrl = beginsWith(appBaseNoFile, url)) ? rewrittenUrl = appBaseNoFile + appUrl : appBaseNoFile == url + "/" && (rewrittenUrl = appBaseNoFile),
      ...
      this.$get = ["$rootScope", "$browser", "$sniffer", "$rootElement", "$window", function($rootScope, $browser, $sniffer, $rootElement, $window) {
        ...
        $rootElement.on("click", function(event) {
            if (html5Mode.rewriteLinks && !event.ctrlKey && !event.metaKey && !event.shiftKey && 2 != event.which && 2 != event.button) {
              ...
              isObject(absHref) && "[object SVGAnimatedString]" === absHref.toString() && (absHref = urlResolve(absHref.animVal)
                  .href),
        ...
        return $browser.onUrlChange(function(newUrl, newState) {
            return isUndefined(beginsWith(appBaseNoFile, newUrl)) ? void($window.location.href = newUrl) : ($rootScope.$evalAsync(function() {
                ...
                newUrl = trimEmptyHash(newUrl),
                  ...
                  defaultPrevented = $rootScope.$broadcast("$locationChangeStart", newUrl, oldUrl, newState, oldState)
                  .defaultPrevented,
            ...
            (initializing || urlOrStateChanged) && (initializing = !1,
              $rootScope.$evalAsync(function() {
                var newUrl = $location.absUrl(),
                  defaultPrevented = $rootScope.$broadcast("$locationChangeStart", newUrl, oldUrl, $location.$$state, oldState)
                  .defaultPrevented;
                $location.absUrl() === newUrl && (defaultPrevented ? ($location.$$parse(oldUrl),
  ...
  function ensureSafeAssignContext(obj, fullExpression) {
    if (obj && (obj === 0..constructor || obj === (!1)
        .constructor || obj === "".constructor || obj === {}.constructor || obj === [].constructor || obj === Function.constructor))
  ...
  function findConstantAndWatchExpressions(ast, $filter) {
    ...
    switch (ast.type) {
      ...
      case AST.CallExpression:
        allConstants = ast.filter ? isStateless($filter, ast.callee.name) : !1,
          argsToWatch = [],
  ...
  function $ParseProvider() {
    ...
    this.$get = ["$filter", function($filter) {
      ...
      function inputsWatchDelegate(scope, listener, objectEquality, parsedExpression, prettyPrintExpression) {
        ...
        for (var oldInputValueOfValues = [], oldInputValues = [], i = 0, ii = inputExpressions.length; ii > i; i++)
      ...
      this.$get = ["$injector", "$exceptionHandler", "$parse", "$browser", function($injector, $exceptionHandler, $parse, $browser) {
        ...
        Scope.prototype = {
          ...
          $new: function(isolate, parent) {
            ...
            return parent = parent || this,
              ...
              parent.$$childHead ? (parent.$$childTail.$$nextSibling = child,
                parent.$$childTail = child) : parent.$$childHead = parent.$$childTail = child, (isolate || parent != this) && child.$on("$destroy", destroyChildScope),
              child
          },
          ...
          $on: function(name, listener) {
            ...
            var self = this;
            return function() {
              var indexOfListener = namedListeners.indexOf(listener); - 1 !== indexOfListener && (namedListeners[indexOfListener] = null,
    ...
    this.aHrefSanitizationWhitelist = function(regexp) {
      ...
      this.$get = function() {
        return function(uri, isImage) {
          ...
          return normalizedVal = urlResolve(uri)
            .href,
    ...
    this.$get = ["$window", "$document", function($window, $document) {
      ...
      if (bodyStyle) {
        ...
        vendorPrefix || (vendorPrefix = "WebkitOpacity" in bodyStyle && "webkit"),
          ...
          animations = !!("animation" in bodyStyle || vendorPrefix + "Animation" in bodyStyle), !android || transitions && animations || (transitions = isString(bodyStyle.webkitTransition),
  ...
  function currencyFilter($locale) {
    ...
    return function(amount, currencySymbol, fractionSize) {
      return isUndefined(currencySymbol) && (currencySymbol = formats.CURRENCY_SYM),
        ...
        null == amount ? amount : formatNumber(amount, formats.PATTERNS[1], formats.GROUP_SEP, formats.DECIMAL_SEP, fractionSize)
  ...
  function getFirstThursdayOfYear(year) {
    var dayOfWeekOnFirst = new Date(year, 0, 1)
      .getDay();
  ...
  function dateFilter($locale) {
    ...
    return function(date, format, timezone) {
      ...
      if (format = format || "mediumDate",
        ...
        isNumber(date) && (date = new Date(date)), !isDate(date) || !isFinite(date.getTime()))
        return date;
      for (; format;)
      ...
      return timezone && (dateTimezoneOffset = timezoneToOffset(timezone, date.getTimezoneOffset()),
        ...
        forEach(parts, function(value) {
          fn = DATE_FORMATS[value],
            text += fn ? fn(date, $locale.DATETIME_FORMATS, dateTimezoneOffset) : value.replace(/(^'|'$)/g, "")
            .replace(/''/g, "'")
        }),
        text
    }
      ...
      return limit = Math.abs(Number(limit)) === 1 / 0 ? Number(limit) : toInt(limit),
        isNaN(limit) ? input : (isNumber(input) && (input = input.toString()),
          isArray(input) || isString(input) ? (begin = !begin || isNaN(begin) ? 0 : toInt(begin),
            begin = 0 > begin ? Math.max(0, input.length + begin) : begin,
            limit >= 0 ? input.slice(begin, begin + limit) : 0 === begin ? input.slice(limit, input.length) : input.slice(Math.max(0, begin + limit), begin)) : input)
    }
  }

  function orderByFilter($parse) {
    function processPredicates(sortPredicate, reverseOrder) {
      return reverseOrder = reverseOrder ? -1 : 1,
        sortPredicate.map(function(predicate) {
          var descending = 1,
            get = identity;
    ...
    function objectValue(value, index) {
      return "function" == typeof value.valueOf && (value = value.valueOf(),
    ...
    function getPredicateValue(value, index) {
      ...
      return null === value ? (type = "string",
        value = "null") : "string" === type ? value = value.toLowerCase() : "object" === type && (value = objectValue(value, index)), {
        value: value,
        type: type
      }
    }
      ...
      attr: function(element, name, value) {
        ...
        if (nodeType !== NODE_TYPE_TEXT && nodeType !== NODE_TYPE_ATTRIBUTE && nodeType !== NODE_TYPE_COMMENT) {
          ...
          if (BOOLEAN_ATTR[lowercasedName]) {
            if (!isDefined(value))
              return element[name] || (element.attributes.getNamedItem(name) || noop)
      ...
      this.$get = ["$$AnimateRunner", "$rootScope", function($$AnimateRunner, $rootScope) {
        ...
        return {
          ...
          push: function(element, event, options, domOperation) {
            return domOperation && domOperation(),
              options = options || {},
    ...
    $CoreAnimateCssProvider = function() {
      this.$get = ["$$rAF", "$q", function($$rAF, $q) {
        ...
        return RAFPromise.prototype = {
            ...
            getPromise: function() {
              return this.defer || (this.defer = $q.defer()),
                this.defer.promise
            },
            then: function(f1, f2) {
  ...
  AST.Program = "Program",
    ...
    AST.prototype = {
      ...
      program: function() {
        for (var body = [];;)
          if (this.tokens.length > 0 && !this.peek("}", ")", ";", "]") && body.push(this.expressionStatement()), !this.expect(";"))
            return {
              type: AST.Program,
              body: body
            }
      ...
      primary: function() {
        ...
        this.expect("(") ? (primary = this.filterChain(),
            this.consume(")")) : this.expect("[") ? primary = this.arrayDeclaration() : this.expect("{") ? primary = this.object() : this.constants.hasOwnProperty(this.peek()
            .text) ? primary = copy(this.constants[this.consume()
            .text]) : this.peek()
          .identifier ? primary = this.identifier() : this.peek()
          .constant ? primary = this.constant() : this.throwError("not a primary expression", this.peek());
        ...
        if ("}" !== this.peekToken()
          ...
          do {
            ...
            property = {
              ...
              this.peek()
              .constant ? property.key = this.constant() : this.peek()
    ...
    ASTCompiler.prototype = {
      compile: function(expression, expensiveChecks) {
        ...
        return this.state = this.stage = undefined,
      ...
      recurse: function(ast, intoId, nameId, recursionFn, create, skipWatchIdCheck) {
        ...
        switch (ast.type) {
          case AST.Program:
            forEach(ast.body, function(expression, pos) {
              self.recurse(expression.expression, undefined, undefined, function(expr) {
                ...
                pos !== ast.body.length - 1 ? self.current()
                .body.push(right, ";") : self.return_(right)
            });
            break;
          case AST.Literal:
                ...
                self.if_(self.notNull(left), function() {
                    ast.computed ? (right = self.nextId(),
                      ...
                      nameId && (nameId.computed = !0,
                        nameId.name = right)) : (ensureSafeMemberName(ast.property.name),
          ...
          case AST.CallExpression:
            ...
            break;
          case AST.AssignmentExpression:
            if (right = this.nextId(),
              left = {}, !isAssignable(ast.left))
      ...
      addEnsureSafeObject: function(item) {
        this.current()
          .body.push(this.ensureSafeObject(item), ";")
      },
      addEnsureSafeMemberName: function(item) {
        this.current()
          .body.push(this.ensureSafeMemberName(item), ";")
      },
      addEnsureSafeFunction: function(item) {
        this.current()
          .body.push(this.ensureSafeFunction(item), ";")
      },
      addEnsureSafeAssignContext: function(item) {
        this.current()
          .body.push(this.ensureSafeAssignContext(item), ";")
      },
      ...
      nextId: function(skip, init) {
        ...
        return skip || this.current()
          .vars.push(id + (init ? "=" + init : "")),
          id
      },
      current: function() {
    ...
    ASTInterpreter.prototype = {
      ...
      recurse: function(ast, context, create) {
        ...
        switch (ast.type) {
          ...
          case AST.Identifier:
            return ensureSafeMemberName(ast.name, self.expression),
              self.identifier(ast.name, self.expensiveChecks || isPossiblyDangerousMemberName(ast.name), context, create, self.expression);
          case AST.MemberExpression:
      ...
      identifier: function(name, expensiveChecks, context, create, expression) {
        return function(scope, locals) {
          ...
          return expensiveChecks && ensureSafeObject(value, expression),
            context ? {
              context: base,
              name: name,
              value: value
      ...
      return ["$timeout", "$parse", function($timeout, $parse) {
        ...
        var formDirective = {
          ...
          compile: function(formElement, attr) {
            formElement.addClass(PRISTINE_CLASS)
              ...
              pre: function(scope, formElement, attr, ctrls) {
                ...
                nameAttr && (setter(scope, controller),
                    attr.$observe(nameAttr, function(newValue) {
                      controller.$name !== newValue && (setter(scope, undefined),
                        controller.$$parentForm.$$renameControl(controller, newValue), (setter = getSetter(controller.$name))(scope, controller))
                    })),
                  formElement.on("$destroy", function() {
      ...
      return {
        ...
        link: function(scope, $element, $attr, ctrl) {
          return /SVG/.test($element[0].toString()) ? ($element.empty(),
            void $compile(jqLiteBuildFragment(ctrl.template, document)
              .childNodes)(scope, function(clone) {
        ...
        this.$$runValidators = function(modelValue, viewValue, doneCallback) {
          ...
          function processAsyncValidators() {
            ...
            forEach(ctrl.$asyncValidators, function(validator, name) {
              ...
              validatorPromises.length ? $q.all(validatorPromises)
              .then(function() {
    ...
    ngModelDirective = ["$rootScope", function($rootScope) {
      return {
        ...
        compile: function(element) {
          return element.addClass(PRISTINE_CLASS)
          ...
          getWatchables: $parse(valuesFn, function(optionValues) {
            ...
            for (var optionValuesKeys = getOptionValuesKeys(optionValues), optionValuesLength = optionValuesKeys.length, index = 0; optionValuesLength > index; index++) {
              var key = optionValues === optionValuesKeys ? index : optionValuesKeys[index],
                locals = (optionValues[key],
                  getLocals(optionValues[key], key)),
      ...
      function ngOptionsPostLink(scope, selectElement, attr, ctrls) {
        ...
        function skipEmptyAndUnknownOptions(current) {
          ...
          if (emptyOption_ || unknownOption_)
            for (; current && (current === emptyOption_ || current === unknownOption_ || current.nodeType === NODE_TYPE_COMMENT || "" === current.value);)
        ...
        function updateOptions() {
          ...
          if (providedEmptyOption && selectElement.prepend(emptyOption),
            ...
            options.items.forEach(function(option) {
              ...
              option.group ? (group = groupMap[option.group],
                group || (groupElement = addOrReuseElement(selectElement[0], currentElement, "optgroup", optGroupTemplate),
                ...
                group.currentOptionElement = optionElement.nextSibling) : (optionElement = addOrReuseElement(selectElement[0], currentElement, "option", optionTemplate),
                updateOptionElement(option, optionElement),
            ...
            ngModelCtrl.$render(), !ngModelCtrl.$isEmpty(previousValue)) {
        ...
        if (ngModelCtrl) {
          for (var emptyOption, selectCtrl = ctrls[0], multiple = attr.multiple, i = 0, children = selectElement.children(), ii = children.length; ii > i; i++)
            if ("" === children[i].value) {
          ...
          multiple ? (ngModelCtrl.$isEmpty = function(value) {
              ...
              ngOptions.trackBy && scope.$watchCollection(function() {
                ...
                option && !option.disabled ? selectElement[0].value !== option.selectValue && (removeUnknownOption(),
                  ...
                  renderEmptyOption()) : (removeEmptyOption(),
                  renderUnknownOption())
              },
              selectCtrl.readValue = function() {
        ...
        link: function(scope, element, attr, ngSwitchController) {
          ...
          scope.$watch(watchExpr, function(value) {
            ...
            selectedElements.length = 0,
              selectedScopes.length = 0, (selectedTranscludes = ngSwitchController.cases["!" + value] || ngSwitchController.cases["?"]) && forEach(selectedTranscludes, function(selectedTransclude) {
    ...
    SelectController = ["$element", "$scope", "$attrs", function($element, $scope) {
      ...
      self.ngModelCtrl = noopNgModelController,
        ...
        self.readValue = function() {
          return self.removeUnknownOption(),
            $element.val()
        },
        self.writeValue = function(value) {
          self.hasOption(value) ? (self.removeUnknownOption(),
            $element.val(value),
            "" === value && self.emptyOption.prop("selected", !0)) : null == value && self.emptyOption ? (self.removeUnknownOption(),
        ...
        self.removeOption = function(value) {
          ...
          count && (1 === count ? (optionsMap.remove(value),
            "" === value && (self.emptyOption = undefined)) : optionsMap.put(value, count - 1))
        },
      ...
      return {
        ...
        compile: function(element, attr) {
          if (isDefined(attr.value))
            var interpolateValueFn = $interpolate(attr.value, !0);
          else {
          ...
          return function(scope, element, attr) {
            var selectCtrlName = "$selectController",
              ...
              selectCtrl = parent.data(selectCtrlName) || parent.parent()
              .data(selectCtrlName);
  ...
  .directive("uiAce", ["uiAceConfig", function(uiAceConfig) {
    ...
    var setOptions = function(acee, session, opts) {
      ...
      angular.isDefined(opts.require) && opts.require.forEach(function(n) {
          window.ace.require(n)
        }),
        angular.isDefined(opts.showGutter) && acee.renderer.setShowGutter(opts.showGutter),
        angular.isDefined(opts.useWrapMode) && session.setUseWrapMode(opts.useWrapMode),
        angular.isDefined(opts.showInvisibles) && acee.renderer.setShowInvisibles(opts.showInvisibles),
        angular.isDefined(opts.showIndentGuides) && acee.renderer.setDisplayIndentGuides(opts.showIndentGuides),
        angular.isDefined(opts.useSoftTabs) && session.setUseSoftTabs(opts.useSoftTabs),
        angular.isDefined(opts.showPrintMargin) && acee.setShowPrintMargin(opts.showPrintMargin),
        angular.isDefined(opts.disableSearch) && opts.disableSearch && acee.commands.addCommands([{
          name: "unfind",
          bindKey: {
            win: "Ctrl-F",
            mac: "Command-F"
        ...
        angular.isString(opts.theme) && acee.setTheme("ace/theme/" + opts.theme),
        angular.isString(opts.mode) && session.setMode("ace/mode/" + opts.mode),
        angular.isDefined(opts.firstLineNumber) && (angular.isNumber(opts.firstLineNumber) ? session.setOption("firstLineNumber", opts.firstLineNumber) : angular.isFunction(opts.firstLineNumber) && session.setOption("firstLineNumber", opts.firstLineNumber()));
      var key, obj;
    ...
    return {
      ...
      link: function(scope, elm, attrs, ngModel) {
        var onChangeListener, onBlurListener, options = uiAceConfig.ace || {},
          opts = angular.extend({}, options, scope.$eval(attrs.uiAce)),
          acee = window.ace.edit(elm[0]),
          session = acee.getSession(),
          executeUserCallback = function() {
            var callback = arguments[0],
              args = Array.prototype.slice.call(arguments, 1);
            angular.isDefined(callback) && scope.$evalAsync(function() {
          ...
          listenerFactory = {
            onChange: function(callback) {
              return function(e) {
                ...
                !ngModel || newValue === ngModel.$viewValue || scope.$$phase || scope.$root.$$phase || scope.$evalAsync(function() {
        ...
        var updateOptions = function(current, previous) {
          current !== previous && (opts = angular.extend({}, options, scope.$eval(attrs.uiAce)),
            opts.callbacks = [opts.onLoad],
            opts.onLoad !== options.onLoad && opts.callbacks.unshift(options.onLoad),
        ...
        scope.$watch(attrs.uiAce, updateOptions, !0),
  ...
  angular.module("jqplay", ["ui.ace", "jqplay.controllers", "jqplay.services", "jqplay.filters"]),
  angular.module("jqplay.controllers", [])
  .controller("JqplayCtrl", function($scope, $timeout, jqplayService) {
    $scope.jq = {},
      $scope.jq.o = {
        "null-input": !1,
        slurp: !1,
      ...
      $scope.editorLoaded = function(_editor) {
        _editor.setHighlightActiveLine(!1),
          ...
          _editor.session.setUseWorker(!1)
      },
      $scope.$watch("jq", function(newValue, oldValue) {
        newValue !== oldValue && $scope.delayedRun($scope.jq)
      }, !0),
      $scope.$watch("jq.o", function(newValue, oldValue) {
        newValue !== oldValue && $scope.delayedRun($scope.jq)
      }, !0),
      $scope.delayedRun = function(jq) {
        $scope.input.$valid ? (null != $scope.runTimeout && ($timeout.cancel($scope.runTimeout),
      ...
      null != window.jq && ($scope.jq.j = window.jq.j,
        $scope.jq.q = window.jq.q,
        $scope.run($scope.jq))
  }),
  angular.module("jqplay.services", [])
  .factory("jqplayService", function($http) {
    return {
      run: function(jq) {
        return $http({
          method: "post",
  ...
  .filter("capitalize", function() {
    return function(input) {
      ...
      input = input.toLowerCase(),
        input = input.replace("-", " ");
      for (var result = [], str = input.split(" "), i = 0; i < str.length; i++)
        result.push(str[i].substring(0, 1)
          .toUpperCase() + str[i].substring(1));
      return result.join(" ")
    }
  });
